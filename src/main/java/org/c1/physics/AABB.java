...
public class AABB {
	...
	public static boolean collides(AABB a, AABB b) {
		if (Math.abs(a.center.x() - b.center.x()) < (a.halfSize.x() + b.halfSize
				.x())) {
			if (Math.abs(a.center.y() - b.center.y()) < (a.halfSize.y() + b.halfSize
					.y())) {
				if (Math.abs(a.center.z() - b.center.z()) < (a.halfSize.z() + b.halfSize
						.z())) {
					return true;
				}
			}
