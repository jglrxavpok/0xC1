...
public class RenderEngine implements IDisposable
{
    ...
    public void flushBuffer(OpenGLBuffer buffer, int mode)
    {
        ...
        C1Client.printIfGLError("Before drawing buffer");
        glDrawElements(mode, buffer.getIndicesCount(), GL_UNSIGNED_INT, 0);
        C1Client.printIfGLError("After drawing buffer");
    ...
    public void renderSplashScreen()
    {
        ...
        buffer.addVertex(Vertex.get(Vector3.get(C1Client.getClient().getDisplayWidth(), 0, 0), Vector2.get(1, 0)));
        buffer.addVertex(Vertex.get(Vector3.get(C1Client.getClient().getDisplayWidth(), C1Client.getClient().getDisplayHeight(), 0), Vector2.get(1, 1)));
        buffer.addVertex(Vertex.get(Vector3.get(0, C1Client.getClient().getDisplayHeight(), 0), Vector2.get(0, 1)));
    ...
    public void switchToOrtho(int w, int h)
    {
        ...
        C1Client.printIfGLError("After switching to ortho, size is " + w + ", " + h);
    }
    ...
    public void bindTexture(ITextureObject object, int slot)
    {
        ...
        C1Client.printIfGLError("after texture bind");
    }
    ...
    {
        setCurrentShader(blitShader);
        C1Client.printIfGLError("before rendering world");
        setCurrentShader(blitShader);
        frameBuffer.bind();
    ...
    {
        C1Client.printIfGLError("after rendering world");
        disableGLCap(GL_DEPTH_TEST);
        switchToOrtho();
        shaderBatch.apply(0, getColorBuffer(), renderBuffer, this);
        C1Client.printIfGLError("after post-processing world");
        setCurrentShader(blitShader);
    }
    ...
    public RenderEngine pushState()
    {
        ...
        C1Client.printIfGLError("after pushing render engine state");
        return this;
    }
    ...
    public RenderEngine popState()
    {
        ...
        pop.apply(this);
        renderState = pop;
        C1Client.printIfGLError("After popping render engine state");
        return this;
    }
    ...
    {
        glEnable(cap);
        C1Client.printIfGLError("after enabling GL cap: " + OpenGLHelper.getCapName(cap));
    ...
    {
        glDisable(cap);
        C1Client.printIfGLError("after disabling GL cap: " + OpenGLHelper.getCapName(cap));
    ...
    {
        this.displayWidth = C1Client.getClient().getDisplayWidth();
        this.displayHeight = C1Client.getClient().getDisplayHeight();
        if(modelMatrix != null)
        {
            modelMatrix.dispose();
