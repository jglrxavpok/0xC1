...
public class RenderState implements Cloneable
{
    ...
    public void apply(RenderEngine renderEngine)
    {
        ...
        if(current != this)
        {
            ...
            C1Client.printIfGLError("after setting up capability from renderState");
            if(blendFuncSrc != -1)
            {
                glBlendFunc(blendFuncSrc, blendFuncDst);
                C1Client.printIfGLError("after setting up blend func from renderState");
            }

            if(alphaFunc != -1)
            {
                glAlphaFunc(alphaFunc, alphaRef);
                C1Client.printIfGLError("after setting up alpha func from renderState");
            }

            glClearColor(clearColorR, clearColorG, clearColorG, clearColorA);
            C1Client.printIfGLError("after setting up clear color from renderState");
